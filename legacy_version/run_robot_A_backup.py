"""
run_robot_A.py  —  Standalone runner for Robot A
=================================================
"""
...
async def play(robot):
    ...
    await robot.set_lights_on_rgb(0, 255, 0)   # Green for A

    world_pts = build_path()
    local_pts = path_in_local(world_pts)

    print(f"Robot A | start={START} seq={SEQ} dir={DIR} | {len(local_pts)} waypoints")
        ...
        if pt["type"] == "move":
            ...
            if prev is None or prev["type"] != "wrap" or prev["pole"] != pt["pole"]:
                ...
                for _ in range(CIRCLES):
                    await robot.arc_left(360, ARC_R)   # CCW
